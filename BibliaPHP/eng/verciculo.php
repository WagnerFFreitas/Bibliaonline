...
<input type="text" hidden name="cap" value="1">
  ...
  $conn = mysqli_connect($servidor, $dbusuario, $dbsenha, $dbname);
   ...
   $sql3 = "SELECT livro from tbbiblia_en GROUP BY liv";
...
<input type="text" hidden name="livro" value="<?php echo $livro;?>">
 <select  name="cap" class="form-control" onchange="document.forms['dataForm3'].submit();">
  ...
  $conn = mysqli_connect($servidor, $dbusuario, $dbsenha, $dbname);
        mysqli_set_charset($conn,"utf8");
  $paginar = "SELECT cap from tbbiblia_en WHERE livro ='$livro' GROUP BY cap";
   $pagi = mysqli_query($conn,$paginar);
   $rowcount=mysqli_num_rows($pagi);
   //
   $sql3 = "SELECT cap from tbbiblia_en WHERE livro ='$livro' GROUP BY cap";
   $resultado3 = mysqli_query($conn,$sql3);
...
<br>
 <div><?php
    ...
    echo '<h4>', $livro, ",", $cap, '</h4>';

    $conn = mysqli_connect($servidor, $dbusuario, $dbsenha, $dbname);
    mysqli_set_charset($conn, "utf8");
    $sql = "SELECT * FROM tbbiblia_en WHERE livro='$livro' AND cap='$cap'";
    ...
    while ($linha = mysqli_fetch_array($resultado)) {
        ...
        if ($versiculo == $ver) {
            $destacado = '<span style="background-color: yellow;">' . $texto . '</span>';
        } else {
            $destacado = $texto;
        }

        echo '<blockquote class="blockquote" style="font-size:23px;">
                <p class="mb-0">' . $versiculo . ' ' . $destacado . '</p>
              </blockquote>';
    }
?>
</div>
